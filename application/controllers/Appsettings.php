...
class Appsettings extends CI_Controller
{
    ...
    public function ubahmidtrans()
    {
        ...
        $this->form_validation->set_rules('id_merchant', 'id_merchant', 'trim|prep_for_form');
        $this->form_validation->set_rules('client_key', 'client_key', 'trim|prep_for_form');
        $this->form_validation->set_rules('server_key', 'server_key', 'trim|prep_for_form');
        $this->form_validation->set_rules('midtrans_status', 'midtrans_status', 'trim|prep_for_form');
        $this->form_validation->set_rules('midtrans_active', 'midtrans_active', 'trim|prep_for_form');

        if ($this->form_validation->run() == TRUE) {
            $data             = [
                'id_merchant'                    => html_escape($this->input->post('id_merchant', TRUE)),
                'client_key'                    => html_escape($this->input->post('client_key', TRUE)),
                'server_key'                => html_escape($this->input->post('server_key', TRUE)),
                'midtrans_status'                        => html_escape($this->input->post('midtrans_status', TRUE)),
                'midtrans_active'                        => html_escape($this->input->post('midtrans_active', TRUE))
            ];
            if (demo == TRUE) {
                $this->session->set_flashdata('demo', 'NOT ALLOWED FOR DEMO');
                redirect('appsettings/index');
                ...
                $this->app->ubahdatamidtrans($data);
                $this->session->set_flashdata('ubah', 'Midtrans berhasil di ubah');
                redirect('appsettings');
            }
        } else {
